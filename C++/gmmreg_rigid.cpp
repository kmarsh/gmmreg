...
void gmmreg_rigid::set_bound()
{
    if (d==2){
        ...
        int i = 0;
        nbd.set_size(7); nbd.fill(0);
        for (i=0;i<4;++i) nbd[i] = 2;
     
        lower_bound.set_size(7);
        lower_bound.fill(0);
        for (i=0;i<4;++i) lower_bound[i] = -1;

        upper_bound.set_size(7);
        upper_bound.fill(0);
        for (i=0;i<4;++i) upper_bound[i] = 1;
    }
}
...
int gmmreg_rigid::set_init_params(const char* f_config)
{
    ...
    GetPrivateProfileString(section, "init_rigid", NULL, f_init_rigid, 80, f_config);
...
void gmmreg_rigid::save_results(const char* f_config, const vnl_vector<double>& params)
{
